...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await resend.emails.send({
      from: 'StoryGennie <onboarding@resend.dev>',
      to: email,
      subject: `${childName}'s Storybook is Ready! 📖 - StoryGennie`,
      html: `
  <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; background: #FFF8EE;">
    
    <div style="text-align: center; margin-bottom: 32px;">
      <h1 style="font-size: 32px; color: #2D1B00; margin: 0;">StoryGennie</h1>
      <p style="color: #9E8060; font-size: 14px; margin: 4px 0 0;">Personalized storybooks for every child</p>
    </div>
    ...
    <div style="text-align: center; margin-top: 24px;">
      <p style="color: #9E8060; font-size: 12px;">
        Made with love by StoryGennie ✨
      </p>
      <p style="color: #9E8060; font-size: 11px; margin-top: 4px;">
        <a href="https://storygennie.com" style="color: #F4A832;">storygennie.com</a>
      </p>
    </div>
